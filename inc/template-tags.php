 ...
 * @return bool
 */
function kulkuri_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'kulkuri_categories' ) ) ) {
		...
		set_transient( 'kulkuri_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so kulkuri_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so kulkuri_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in kulkuri_categorized_blog.
 */
function kulkuri_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'kulkuri_categories' );
}
add_action( 'edit_category', 'kulkuri_category_transient_flusher' );
add_action( 'save_post',     'kulkuri_category_transient_flusher' );
